...
function GlossaryPage() {
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginTop: 24, color: "var(--navy-soft)", fontSize: 14 }}>{filtered.length} {filtered.length === 1 ? "pojem" : filtered.length < 5 ? "pojmy" : "pojmů"}</div>
        ...
        {filtered.map((g, i) => (
          <div key={i} className="glossary-item">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
              ...
              <span className="pill" style={{ fontSize: 11 }}>{g.cat}</span>
            </div>
            <p style={{ marginTop: 4, fontSize: 15, color: "var(--navy-soft)" }}>{g.def}</p>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="card" style={{ gridColumn: "1 / -1", textAlign: "center", padding: 40 }}>
            <Mascot size={60} mood="wink"/>
            <p style={{ marginTop: 12, fontWeight: 700 }}>Nic jsem nenašel. Zkus jiné slovíčko!</p>
          </div>
        )}
      </div>
    </div>
