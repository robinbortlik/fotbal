...
function PositionsPage() {
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 22 }} className="hero-grid">
        <div className="pitch-wrap">
          <PitchSVG>
            {POSITIONS.map(pos => (
              <g key={pos.id} onClick={() => setActive(pos.id)} style={{ cursor: "pointer" }}>
                {active === pos.id && <circle cx={pos.x} cy={pos.y} r="5.5" fill="none" stroke="#F2A007" strokeWidth="0.6" strokeDasharray="1 0.8"/>}
                <circle cx={pos.x} cy={pos.y} r={active === pos.id ? 4 : 3.2} fill={pos.color} stroke="#0B1F33" strokeWidth="0.5"/>
                <text x={pos.x} y={pos.y + 1} textAnchor="middle" fontSize="2" fontWeight="800" fill="#0B1F33" fontFamily="Bricolage Grotesque, sans-serif">{pos.short.split(" / ")[0]}</text>
              </g>
            ))}
          </PitchSVG>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="card" style={{ background: p.color, borderColor: "var(--navy)", padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ fontSize: 56 }}>{p.emoji}</div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>{p.short}</div>
                <div className="display" style={{ fontSize: 26 }}>{p.name}</div>
              </div>
            </div>
            <p style={{ marginTop: 14, fontSize: 16 }}>{p.desc}</p>
          </div>
          <div className="card">
            <div className="display" style={{ fontSize: 18 }}>Co musíš umět</div>
            <div className="pill-row" style={{ marginTop: 10 }}>
              {p.skills.map((s, i) => <span key={i} className="pill orange">{s}</span>)}
            </div>
          </div>
          <div className="card dark">
            <div className="display" style={{ fontSize: 18, color: "var(--orange)" }}>Slavní hráči</div>
            <ul style={{ paddingLeft: 18, lineHeight: 1.8, marginTop: 6, fontSize: 15 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
        {POSITIONS.map(pos => (
          <button key={pos.id} className="tile" onClick={() => setActive(pos.id)} style={{
            padding: 14,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ textAlign: "left" }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em" }}>{pos.short}</div>
                <div style={{ fontFamily: "Bricolage Grotesque, sans-serif", fontWeight: 800, fontSize: 15, lineHeight: 1.1 }}>{pos.name}</div>
              </div>
            </div>
          </button>
        ))}
...
Object.assign(window, { PositionsPage });
